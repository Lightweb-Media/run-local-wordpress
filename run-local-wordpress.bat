...
php.exe wp-cli.phar core download --locale="de_DE"

:: You can later install another language package
:: php.exe wp-cli.phar core language es_ES --activate

:: This part of the config is a bit dummy, because it will be overrided by sqlite plugin
php.exe wp-cli.phar core config --skip-check --dbname=wp --dbuser=wp --dbpass=pass

:: Go to wp-content directory, then override db.php controller
cd wp-content

:: Download sqlite integration plugin
bitsadmin.exe /transfer "DownloadSQLite" https://raw.githubusercontent.com/aaemnnosttv/wp-sqlite-db/master/src/db.php db.php

:: Return to main directory.
cd ../

:: Install downloaded wordpress, set real data and set desired url:port
php.exe wp-cli.phar core install --title=Dev --admin_name=admin --admin_email="admin@example.com" --admin_password=pass --url="http://localhost:8080/"
